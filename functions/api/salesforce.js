export async function onRequest(context) {
  ...
  const { refreshToken, instanceUrl, action } = await request.json();
  if (!refreshToken || !instanceUrl) return new Response(JSON.stringify({ error: 'Missing refreshToken or instanceUrl' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
