export async function onRequest(context) {
  ...
  const { clientId, clientSecret, username, password, instanceUrl, action } = await request.json();
  if (!instanceUrl || !username || !password) return new Response(JSON.stringify({ error: 'Missing Salesforce credentials' }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
  ...
  if (tokenData.error) return new Response(JSON.stringify({ error: tokenData.error_description || tokenData.error }), { status: 401, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
  ...
  try {
    if (action === 'opportunities') {
      ...
      if (data.errorCode) return new Response(JSON.stringify({ error: data.message || data.errorCode }), { status: 400, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
